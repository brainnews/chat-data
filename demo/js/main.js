...
var ShowLoadingStatus = function(str) {
	$('#loading-status').text(str);
}
...
var SortMembers = function(sortType) {
	ShowLoadingStatus('Sorting members');
...
var GetGroupChatName = function(text) {
	ShowLoadingStatus('Sourcing group chat name');
	var groupChatName = s.strRight(text[1], 'group ');
	//widdle down the group chat name
...
var GetMembers = function(text) {
	ShowLoadingStatus('Extracting members');
	var members = [];
	var testArray = [];
	...
	for (var i = 0; i < memberNames.length; i++) {
		...
		if (memberList.indexOf(detectedName) < 0) {
			...
			//chatObj.members[memberNumber].name = memberNames[i];
...
var GetMessages = function(text) {
	ShowLoadingStatus('Sifting conversation');
	for (var i = 0; i < Object.keys(chatObj.members).length; i++) {
		var memberNum = i;
		var wordVomit = '';
		$.each(text, function(i,l){
...
var RandomQuote = function(member) {
	//ShowLoadingStatus('Finding random quotes');
	var quoteArray = [];
	if (member) {
		for (var i = 0; i < Object.keys(chatObj.members).length; i++) {
